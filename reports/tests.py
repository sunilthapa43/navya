...
class ReportsAPITest(APITestCase):
    def setUp(self):
        ...
        MutualFund.objects.create(
            name="Mutual Fund 1", fund_type="Equity", symbol="MF1", nav=2532551.22
        )
    ...
    def test_reports_endpoint(self):
        ...
        response = self.client.get(
            reports_url, HTTP_AUTHORIZATION=f"Bearer {self.access_token}"
        )
    ...
    def test_mutual_fund_creation_by_non_admin(self):
        ...
        self.assertIn(
            "You are not authorized to perform this action", response.data["error"]
        )
